...
PRODUCT="BMO Saturator"
BUILD=build/BMOSaturator_artefacts/RelWithDebInfo
...
stale=0
for pair in "VST3/$PRODUCT.vst3:VST3/$PRODUCT.vst3" "AU/$PRODUCT.component:Components/$PRODUCT.component"; do
    built="$BUILD/${pair%%:*}/Contents/MacOS/$PRODUCT"
    installed="$INSTALL/${pair##*:}/Contents/MacOS/$PRODUCT"
...
if (( stale )); then
    ...
    echo "  cmake --build build --target BMOSaturator_VST3 BMOSaturator_AU"
    exit 1
fi

[[ ${1:-} == --test ]] && ctest --test-dir build --output-on-failure
[[ ${1:-} == --run  ]] && open "$BUILD/Standalone/$PRODUCT.app"
