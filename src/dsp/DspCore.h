...
/** Everything the plugin does to audio, with no dependency on JUCE's plugin
    ...
    The chain, per channel, inside the oversampled region:
        ...
        input trim -> asymmetric curve -> the distortion it made, split in two
                   -> summed back over the dry signal -> DC blocker
                   -> makeup -> output trim

    The unusual step is the third one, and it is the whole reason this hits the
    ...
    So the curve's output is split into the part that was already there and the
    part it added -- the residual, f(x) - x, which is the distortion and
    nothing else -- and only the residual is filtered. Below kResidualSplitHz
    it is added back at the weight the curve was fitted with, so the low and
    mid bands behave exactly as the fitted curve says they should. Above it, it
    is added back harder, with a further shelf tilting the top two octaves up.
    The harmonics are still harmonics of the programme, generated by an
    asymmetric curve and carrying its even-order signature; they are simply
    weighted towards the part of the spectrum the target puts them in.
...
public:
    ...
    /** DRIVE, 0-100 on the panel, mapped to the shaper's positive-half drive.
